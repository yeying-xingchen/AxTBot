...
from src.Utils.EventClass import GroupMessageEvent, MarkdownPayload
...
@command(["jrrp", "/jrrp"])
async def jrrp_handler(event: GroupMessageEvent):
    result, code = await get_jrrp(event.user_id)
    if code == 2:
        await event.reply(markdown=result)
    else:
...
custom = False  # 开关 *
cusjrrp = 100  # jrrp值 *
custom1 = "您今日的人品值是：2025！！！"  # 段落1
custom2 = "AxT 团队在此祝福大家新年快乐~"  # 段落2
custom3 = (
    "在这喜庆的日子，祝您在新的一年里：万事如意，心想事成，顺风顺水顺财神！"  # 段落3
)
...
# markdown 模板ID
custom_template_id = "xxxxxxxxxx_xxxxxxxx"

# 数据库名称
default_database = "./data/jrrp.db"
custom_database = "./data/customjrrp.db"

# ----------------------------预设值----------------------------  结束


# ----------------------------主代码----------------------------  开始
async def get_jrrp(member_id):  # 主操作
...
async def update_jrrp(memid, jrrp):  # 写数据库信息
    ...
    cursor.execute(
        f"SELECT number FROM {table_name} WHERE id = ?", (memid,)
    )  # 查询是否存在此人的信息记录
...
您的人品值是: {number}""",
    ...
    # 详见https://bot.q.qq.com/wiki/develop/api-v2/server-inter/message/send-receive/send.html#群聊
        ...
        cursor.execute(
            f"INSERT INTO {table_name} (id, number) VALUES (?, ?)", (memid, jrrp)
        )  # 插入信息
        conn.commit()  # 提交
        conn.close()  # 关闭
...
==================""",
            ...
            return (
                await content_load(jrrp),
                2,
            )  # 返回这个函数的值，这个函数会通过jrrp判断要写的内容并返回一个markdown
...
async def update_database():
    ...
    cursor.execute(
        f"CREATE TABLE IF NOT EXISTS {table_name} (id TEXT PRIMARY KEY, number INTEGER)"
    )  # 新开一个
    conn.commit()  # 提交
    conn.close()  # 关闭
...
async def content_load(jrrpnumber):  # 通过jrrp值判断输出内容
    ...
    if paragraph1 == 0:
        ...
        return markdown  # 返回这个值用于发送markdown
...
async def jrrp_note(jrrpnumber):
    if custom:  # 特殊的地方
        ...
        print(
            f"未知的jrrp参数：{jrrpnumber}"
        )  # 若确实如此，直接返回此内容并返回00000，不再运行
        return 0, 0, 0, 0, 0
